...
function spawnEnemy(state: CoreState, spec: EnemySpec, distance: number): void {
  ...
  state.enemies.push({
    ...
    holdChargeTicks: 0,
    aliveTicks: 0,
    weaponKind: spec.weaponKind ?? null,
    shieldKind: spec.shieldKind ?? null,
    generatorKind: spec.generatorKind ?? 'none',
    motorKind: spec.motorKind ?? 'steady',
    holdBonusTiered: spec.holdBonusTiered ?? false,
    shield: spec.shieldCapacity ?? 0,
    shieldCapacity: spec.shieldCapacity ?? 0,
    displayName: spec.displayName ?? spec.kind.toUpperCase(),
    rearWeaponKind: spec.rearWeaponKind ?? null,
    rearShotDamage: spec.rearShotDamage ?? 0,
    rearTicksBetweenShots: spec.rearTicksBetweenShots ?? 0,
    rearCritChance: spec.rearCritChance ?? 0,
    rearMissChance: spec.rearMissChance ?? 0,
    rearCritMult: spec.rearCritMult ?? 2.0,
    rearShootTimer: spec.rearTicksBetweenShots ?? 0,
  });
}
